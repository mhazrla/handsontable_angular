...
function matches(role: RolePermission, term: string, pipe: PipeTransform) {
  if (!role) {
    return false;
  }

  const role_name = role.role_name ? role.role_name.toLowerCase() : '';
  const permission_code = role.permission_code
    ? role.permission_code.toLowerCase()
    : '';
...
export class RoleService {
  ...
  constructor(private pipe: DecimalPipe, public apiService: ApiService) {
    ...
    // Api Data
    this.apiService.getAll('admin/user-data/roles/').subscribe((role) => {
      this.datas = role.data;
    });
  }
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 2. filter
    countries = countries.filter((role) =>
      matches(role, searchTerm, this.pipe)
    );
