...
@Component({
  selector: 'app-create-fb60',
  templateUrl: './create-fb60.component.html',
  styleUrls: ['./create-fb60.component.scss'],
})
export class CreateFb60Component implements OnInit {
  Default = [{ name: '1' }, { name: '2' }, { name: '3' }];
  userLogged: any;
  ...
  ngOnInit() {
    ...
    this.fb60Form = this.formBuilder.group({
      companyCode: [null, [Validators.required, Validators.maxLength(4)]],
      vendor: [null, [Validators.required, Validators.maxLength(10)]],
      documentDate: ['', [Validators.required]],
      postingDate: ['', [Validators.required]],
      period: [{ value: '', disabled: true }, [Validators.maxLength(2)]],
      currency: [null, [Validators.required, Validators.maxLength(5)]],
      amount: ['', [Validators.required, Validators.maxLength(15)]],
      busPlace: ['', [Validators.required, Validators.maxLength(4)]],
      text: ['', Validators.maxLength(50)],
      documentType: [
        { value: this.initialDocumentTypeValue, disabled: true },
        Validators.required,
      ],
      reference: ['', [Validators.required, Validators.maxLength(16)]],
      headerText: ['', [Validators.required, Validators.maxLength(25)]],
      baselineDate: [''],
      payterm: [null, Validators.maxLength(4)],
      partBank: [null, [Validators.required, Validators.maxLength(4)]],
      taxVATCode: [null, Validators.maxLength(2)],
      taxVATAmount: ['', Validators.maxLength(15)],
      whtType: [null, Validators.maxLength(2)],
      whtBaseAmount: ['', Validators.maxLength(15)],
      whtCode: [null, Validators.maxLength(2)],
      whtAmount: ['', Validators.maxLength(15)],
        ...
        if (source === 'edit') {
          for (const change of changes) {
            ...
            if (col === 2 && !dropdownSourceDc.includes(newValue)) {
              ...
              col === 1 &&
              newValue.length > 50 // Short Text max length
            ) {
              Swal.fire({
                title: 'Invalid input',
  ...
  openModal(content: any, row: number) {
    ...
    this.profitSegmentForm.patchValue({
      ...
      companyCode: this.fb60Form.value.companyCode,
    });
  ...
  submitHeader(status: string): Observable<number> {
    this.fb60Form.get('documentType')?.enable();
    this.fb60Form.get('period')?.enable();
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          if (this.fb60Form.valid) {
            this.apiService
              .post('journal/fb60/', this.fb60Form.value)
              .subscribe((res: any) => {
