...
export class F02Component implements OnInit, OnDestroy {
  ...
  fetchF02() {
    fetch('https://jsonplaceholder.typicode.com/todos/')
      ...
      .then((json) => {
        this.f02s = json;
        this.paginationService.calculateTotalPages(this.f02s.length);
        this.paginateData();
        this.initializeHandsontable();
      });
    // this.apiService.getAll('journal/f02/').subscribe(
    //   (res) => {
    //     this.f02s = res.data;
    //     this.paginationService.calculateTotalPages(this.f02s.length);
    //     this.paginateData();
    ...
    //   },
    //   (error) => {
    //     console.log('Error on fetchin F02 Journal', error);
    //   }
    // );
  }
  ...
  initializeHandsontable() {
    ...
    let data = this.f02sToShow.map((journal) => [
      ...
      journal.status,
      '',
      journal.id_f02_header,
    ]);

    let mergeCellsConfig: any[] = [];
    if (data.length === 0) {
      data = [['No Data', '', '', '', '', '', '', '', '', '']];
      mergeCellsConfig = [{ row: 0, col: 0, rowspan: 1, colspan: 9 }];
    }
      ...
      columns: [
        ...
        { readOnly: true },
        {
          ...
          renderer: (
            ...
            const status = instance.getDataAtRowProp(row, 7);
            const idHeader = instance.getDataAtRowProp(row, 9);
            ...
            const handleEditOrShow = (event: MouseEvent) => {
              this.router.navigate(['journal/f02/edit/', idHeader]);
            };
            ...
            if (status === 'Draft') {
              ...
              reverseSvg.style.marginRight = '8px';
  ...
  openModal(content: any, row: number, idHeader: string) {
    ...
    this.reverseForm.patchValue({
      ...
      companyCode: this.reverseForm.value.companyCode,
    });
  ...
  paginateData() {
    this.f02sToShow = this.paginationService.paginateData(
      this.f02s,
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        if (this.reverseForm.valid) {
          ...
          this.apiService
            .put('journal/f02/reverse/', formValues.idHeader, formValues)
            .subscribe((response) => {
              this.modalService.dismissAll();
              Swal.fire(
