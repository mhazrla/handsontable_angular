...
@Component({
  selector: 'app-fb50',
  templateUrl: './fb50.component.html',
  styleUrls: ['./fb50.component.scss'],
  providers: [PaginationService],
})
export class Fb50Component implements OnInit, OnDestroy {
  ...
  fetchFb50() {
    fetch('https://jsonplaceholder.typicode.com/todos/')
      ...
      .then((json) => {
        this.fb50s = json;
        this.paginationService.calculateTotalPages(this.fb50s.length);
        this.paginateData();
        this.initializeHandsontable();
      });

    // this.apiService.getAll('journal/fb50/').subscribe(
    //   (res) => {
    //     this.fb50s = res.data;
    //     this.paginationService.calculateTotalPages(this.fb50s.length);
    //     this.paginateData();
  ...
  initializeHandsontable() {
    ...
    let data = this.fb50sToShow.map((journal) => [
        ...
        { readOnly: true },
        {
          ...
          renderer: (
            ...
            const handleEditOrShow = (event: MouseEvent) => {
              this.router.navigate(['journal/fb50/edit/', idHeader]);
            };
  ...
  paginateData() {
    this.fb50sToShow = this.paginationService.paginateData(
      this.fb50s,
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        if (this.reverseForm.valid) {
          ...
          this.apiService
            .put('journal/fb50/reverse/', formValues.idHeader, formValues)
            .subscribe((response) => {
              this.modalService.dismissAll();
              Swal.fire(
