...
@Component({
  selector: 'app-edit-fb50',
  templateUrl: './edit-fb50.component.html',
  styleUrls: ['./edit-fb50.component.scss'],
})
export class EditFb50Component implements OnInit {
  ...
  ngOnInit() {
    ...
    this.fb50Form = this.formBuilder.group({
      companyCode: [
      ...
      period: [{ value: { value: '', disabled: isReadOnly }, disabled: true }],
      year: [{ value: '', disabled: isReadOnly }, [Validators.required]],
    ...
    this.route.paramMap.subscribe((params) => {
      ...
      if (!this.idHeader) {
        this.router.navigate(['/journal/fb50']);
      } else {
        this.loadData();
      }
    });
  ...
  loadData() {
    if (this.idHeader) {
      this.apiService.getSingle(`journal/fb50/`, this.idHeader).subscribe(
        (res: any) => {
          this.fb50Details = res.data.details;
          this.fb50Status = res.data.fb50.status;
          const documentDate = moment(
            res.data.fb50.document_date,
            'DD.MM.YYYY'
          );
          const formattedDocumentDate = documentDate.format('YYYY-MM-DD');
          const postingDate = moment(res.data.fb50.posting_date, 'DD.MM.YYYY');
          const formattedPostingDate = postingDate.format('YYYY-MM-DD');

          this.fb50Form.patchValue({
            companyCode: res.data.fb50.company_code,
            documentDate: formattedDocumentDate,
            postingDate: formattedPostingDate,
            headerText: res.data.fb50.header_text,
            reference: res.data.fb50.reference,
            documentType: res.data.fb50.document_type,
            period: res.data.fb50.period,
            year: res.data.fb50.year,
            currency: res.data.fb50.currency,
            busPlace: res.data.fb50.bus_place,
            status: res.data.fb50.status,
          });
  ...
  initializeHandsontable() {
    ...
    const data =
      this.fb50Details.length > 0
        ? [
            ...this.fb50Details.map((detail) => [
              detail.gl_account,
              detail.short_text,
              detail.d_c,
        ...
        if (source === 'edit') {
          for (const change of changes) {
            ...
            if (col === 2 && !dropdownSourceDc.includes(newValue)) {
              ...
              change[3] = oldValue;
            } else if (col === 4 && !dropdownSourceTaxCode.includes(newValue)) {
              Swal.fire({
                title: 'Invalid input',
      ...
      columns: [
        ...
        { readOnly: isReadOnly },
        {
          ...
          renderer: (
            ...
            const profitSegment = instance.getDataAtRowProp(row, 10);
  ...
  openModal(content: any, row: number) {
    ...
    this.profitSegmentForm.patchValue({
      ...
      companyCode: this.fb50Form.value.companyCode,
    });
  ...
  submitHeader(status: string): Observable<number> {
    this.fb50Form.get('documentType')?.enable();
    this.fb50Form.get('period')?.enable();
    ...
    return new Observable((observer) => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          if (this.fb50Form.valid) {
            this.apiService
              .put('journal/fb50/', this.idHeader, this.fb50Form.value)
              .subscribe((res: any) => {
                this.fb50Form.reset({
                  documentType: {
  ...
  submitDetail(idHeader: number, status: string) {
    ...
    this.apiService
      .post('journal/fb50/detail', journalDetail)
      .subscribe((response) => {
        this.modalService.dismissAll();
